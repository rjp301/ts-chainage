...
class Centerline:
    ...
    def __post_init__(self):
        assert (
            self.value_col in self.points.columns
        ), "value_col must be a field in points file"

        self.points = (
            self.points.to_crs(self.crs)
        ...
        self.line = self.line_data.to_crs(self.crs).geometry.unary_union
        self.line = (
            linemerge(self.line)
            if self.line.geom_type == "MultiLineString"
            else self.line
        )
    ...
    def perp_angle(self, node):
        assert (
            self.offset
        ), "Must generate offset at instantiation to find perpindicular angle"
        moved_pt = self.move_to_ln(node)
        offset_pt = self.offset.interpolate(self.offset.project(node))
        return math.degrees(
            math.atan2(offset_pt.y - moved_pt.y, offset_pt.x - moved_pt.x)
        )
...
def import_CL(which="TM5B.13", import_fp=False, **kwargs):
    root_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "data")

    centerlines = {
        "TM5B.13": {
            "fname_ln": os.path.join(
                root_path, "TMEP_S5_5.24.013", "CNTRLINE_SSEID005p24p13.shp"
            ),
            "fname_pt": os.path.join(
                root_path, "TMEP_S5_5.24.013", "MRKRS_SSEID005p24p13.shp"
            ),
            "fname_fp": os.path.join(
                root_path, "TMEP_S5_5.24.013", "FTPRINT_SSEID005p24p13.shp"
            ),
            "value_col": "Measure",
        ...
        "TM5B.9": {
            "fname_ln": os.path.join(
                root_path, "TMEP_S5_5.24.009", "CNTRLINE_SSEID005p24p9.shp"
            ),
            "fname_pt": os.path.join(
                root_path, "TMEP_S5_5.24.009", "MRKRS_SSEID005p24p9.shp"
            ),
            "fname_fp": os.path.join(
                root_path, "TMEP_S5_5.24.009", "FTPRINT_SSEID005p24p9.shp"
            ),
            "value_col": "Measure",
            "native_crs": "EPSG:26910",
        },
        "TM5B.7": {
            "fname_ln": os.path.join(
                root_path, "TMEP_S5_5.24.007", "CNTRLINE_SSEID005p24p7.shp"
            ),
            "fname_pt": os.path.join(
                root_path, "TMEP_S5_5.24.007", "MRKRS_SSEID005p24p7.shp"
            ),
            "fname_fp": os.path.join(
                root_path, "TMEP_S5_5.24.007", "FTPRINT_SSEID005p24p7.shp"
            ),
            "value_col": "Measure",
            "native_crs": "EPSG:26910",
        },
        "TM5B.6": {
            "fname_ln": os.path.join(
                root_path, "TMEP_S5_5.24.006", "CNTRLINE_SSEID005p24p6.shp"
            ),
            "fname_pt": os.path.join(
                root_path, "TMEP_S5_5.24.006", "MRKRS_SSEID005p24p6.shp"
            ),
            "fname_fp": os.path.join(
                root_path, "TMEP_S5_5.24.006", "FTPRINT_SSEID005p24p6.shp"
            ),
            "value_col": "Measure",
            "native_crs": "EPSG:26910",
        },
        "TM5B.5": {
            "fname_ln": os.path.join(
                root_path, "TMEP_S5_5.24.005", "CNTRLINE_SSEID005p24p5.shp"
            ),
            "fname_pt": os.path.join(
                root_path, "TMEP_S5_5.24.005", "MRKRS_SSEID005p24p5.shp"
            ),
            "fname_fp": os.path.join(
                root_path, "TMEP_S5_5.24.005", "FTPRINT_SSEID005p24p5.shp"
            ),
            "value_col": "Measure",
            "native_crs": "EPSG:26910",
        },
        "TM5B.2": {
            "fname_ln": os.path.join(
                root_path, "TMEP_S5_5.24.002", "CNTRLINE_SSEID005p24p2.shp"
            ),
            "fname_pt": os.path.join(
                root_path, "TMEP_S5_5.24.002", "MRKRS_SSEID005p24p2.shp"
            ),
            "fname_fp": os.path.join(
                root_path, "TMEP_S5_5.24.002", "FTPRINT_SSEID005p24p2.shp"
            ),
            "value_col": "Measure",
            "native_crs": "EPSG:26910",
        },
        "TM1": {
            "fname_ln": os.path.join(
                root_path, "TMPL_ED41.35628", "line1_kmz_line.shp"
            ),
            "fname_pt": os.path.join(
                root_path, "TMPL_ED41.35628", "line1_kmz_points.shp"
            ),
            "fname_fp": None,
            "value_col": "measure",
            "native_crs": "EPSG:26910",
        ...
        "DVS": {
            "fname_ln": os.path.join(root_path, "Macro_DVS", "DVS - CL.shp"),
            "fname_pt": os.path.join(root_path, "Macro_DVS", "DVS - KPs.shp"),
            "fname_fp": os.path.join(root_path, "Macro_DVS", "DVS - FP.shp"),
            "value_col": "STATION",
            "native_crs": "EPSG:32611",
        },
        "Colt": {
            "fname_ln": os.path.join(root_path, "Macro_Colt", "Colt - CL.shp"),
            "fname_pt": os.path.join(root_path, "Macro_Colt", "Colt - KPs.shp"),
            "fname_fp": os.path.join(root_path, "Macro_Colt", "Colt - FP.shp"),
            "value_col": "STATION",
            "native_crs": "EPSG:32611",
        },
    }
    ...
    data_fp = (
        gpd.read_file(centerlines[which]["fname_fp"])
        if centerlines[which]["fname_fp"] and import_fp
        else None
    )
